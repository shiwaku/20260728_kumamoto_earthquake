...
- 配色は気象庁の震度階級色（`#b40068` 震度7 〜 `#fae696` 震度4）。配信 PNG から実測した値
...
| レイヤー | 色相 |
| --- | --- |
| 推計震度（気象庁） | 震度7 356° / 6強 23° / 6弱 31° / 5強 65° / 5弱 101° / 震度4 96° |
| 主要活断層帯の線 | 258° |
| **夜間人口** | **310°** |

で、夜間人口は最も近い震度7（356°）から45.8°、活断層線（258°）から52.1°離れている。
...
現行は薄端 2.11:1（淡色）/ 7.72:1（暗）、濃端 7.38:1（淡色）/ 2.20:1（暗）で、
...
| 区切り | 各階級の占有率 |
| --- | --- |
| 旧 1/10/30/100/300/1000 | 45.0 / 26.6 / 19.9 / 7.8 / **0.7** / **0.0** % |
| 現行 1/5/15/40/100/250 | 25.2 / 30.7 / 21.2 / 14.5 / 7.4 / 1.0 % |
...
右下の「3D」で立体表示に切り替わる（`fill-extrusion`、1人あたり5m、pitch 45°）。
色の明暗より高さのほうが量の差が読みやすいので、どのメッシュに人が多いかを見るときはこちら。
立体化するレイヤーは 2D でも `fill-extrusion` のまま高さ0で描いており、
切替でレイヤーを載せ直す必要がない。
