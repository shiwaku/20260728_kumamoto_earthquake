...
配信元が CORS を返すレイヤーは実行時に直接参照し、同梱は[5つだけ](#同梱データ)。
...
| 変位境界<br>25本・7.5KB | 配信元が `www.gsi.go.jp` の ZIP で CORS を返さない。座標が空の26地物を落として線色を付けている | `tools/build_displacement_boundary.py` |
...
- **空中写真から判読した変位境界（八代地区）** … 7/31公開だが[PDF](https://www.gsi.go.jp/chirijoho/chirijoho41071.html)
  だけで、タイルも GeoJSON も配信されていない。八代平野の東端に沿って北東-南西方向に伸び、
  最大2.1mの右横ずれが計測されている（SAR から判読したほうの変位境界は収録済み）
