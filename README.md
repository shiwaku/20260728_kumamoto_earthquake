...
| 地形・活断層 | 3D地形（シームレス標高タイル） | Terrain-RGB | 産総研 地質調査総合センター | |
...
- `terrain` … 3D地形。レイヤーではなく `map.setTerrain()` で効かせるので描画レイヤーを持たない。
  スライダーは `slider` で標高の誇張率に振り替えている（既定は不透明度）
...
- ONにすると `setView3D(true)` を通して視点を倒す。独自に `easeTo` しないのは、
  右下の 2D/3D ボタンの表示と実際の傾きが食い違わないようにするため
...
右下の「3D」で立体表示に切り替わる（`fill-extrusion`、1人あたり5m、pitch 45°／地形ONなら62°）。
