...
const SUICHOKU_SET: { key: string; id: string; name: string; z: number; desc: string }[] = [
  ...
  {
    key: 'suichoku-keno',
    id: '20260729kumamoto_kumamotokeno_0812suichoku_sokuho',
    name: '垂直写真（速報）熊本県央地区 8/12撮影',
    z: 61,
    desc:
      '御船町・甲佐町から山都町にかけての438枚。写真番号 24986〜25738、8/12 8:46〜10:15の撮影。' +
      '速報用の一眼カメラ（画像URLが .../0812kumamotoken-o_ichigan/...）のため画質が低い。' +
      '地震から2週間後の撮影で、他の地区より東側を写している。',
  },
]
...
export const LAYERS: LayerDef[] = [
  ...
  },
  {
    kind: 'raster',
    key: 'ortho-keno',
    name: '正射画像（速報）熊本県央地区 8/12撮影',
    group: '空中写真',
    on: false,
    opacity: 1,
    z: 23,
    tiles: ['https://maps.gsi.go.jp/xyz/20260729kumamoto_kumamotokeno_0812do_sokuho/{z}/{x}/{y}.png'],
    minzoom: 10,
    maxzoom: 18,
    tileSize: 256,
    // z13 のタイル到達確認（360枚を走査して36枚が200）から求めた実測 bbox。
    // 熊本3地区・熊本4地区の東隣、御船町・甲佐町から山都町へ延びる南北に広い範囲。
    bounds: [130.6934, 32.4727, 131.0889, 32.7318],
    attribution: GSI_ATTR,
    desc:
      '速報用の一眼カメラで撮った写真から自動処理で作成した正射画像のため、他の地区より画質が低い。' +
      '構造物等に歪み・ズレ・不連続が生じて見えることがあり、雲で地表が見えにくい範囲もある。' +
      '地震から2週間後の8/12撮影で、他の正射画像がまだ覆っていない山都町方面まで写している。',
  },
  // 垂直写真は地区×撮影日で12レイヤーある。定義は SUICHOKU_SET を見る。
  ...SUICHOKU_LAYERS,
  {
    kind: 'geojson',
    render: 'point',
